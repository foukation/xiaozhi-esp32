...
typedef enum {
    ...
    DUER_HTTP_ERR_BODY_SIZE,     // response body is too large to fit in memory
} duer_http_result_t;
 ...
 *
 * DESC:
 * the callback to handler the whole body download by http
 *
 * PARAM:
 * @param[in] p_user_ctx: usr ctx registed by user
 * @param[in] buf:   buffer stored the whole HTTP response body
 * @param[in] len:   data length of buf
 * @param[in] type:  data type to identify media or others
 *
 * @RETURN: negative number when failed
 */
typedef int (*duer_http_body_handler)(void *p_user_ctx, const char *buf, size_t len, const char *type);
...
typedef struct {
    ...
    char                      *p_chunk_buf;
    size_t                     recv_size;            // size of the data have been download
    ...
    duer_mutex_t               lock;
    int                        conn_wait_close;      // the long connect is waiting to be closed

    // If data_hdlr_cb is set, the following fields is unused.
    duer_http_body_handler     body_hdlr_cb;         // callback when whole response body is downloaded
    void                      *p_body_hdlr_ctx;      // users args for body_hdlr_cb
    size_t                     max_body_size;        // maximum body size to alloc, when body size exceeds this value, the request may fail
    size_t                     body_size;            // size of body_buffer is body_size + 1
    char                      *body_buffer;          // buffer to store concat body
} duer_http_client_t;
...
/**
 * DESC:
 * register callback to handle response body
 *
 * @param[in] p_client: pointer of the http client
 * @param[in] body_hdlr_cb: response body handler callback to be registered
 * @param[in] p_usr_ctx: user context for response body handler
 *
 * @RETURN    none
 */
void duer_http_reg_body_hdlr(duer_http_client_t *p_client,
                             duer_http_body_handler body_hdlr_cb,
                             void *p_usr_ctx);
...
/**
 * DESC:
 * set maximum body size for request.
 * Pass 0 to turn off limit.
 * 
 * PARAM:
 * @param[in] p_client: pointer point to the duer_http_client_t
 * @param[in] max_body_size: limit of body size in response body concat.
 * 
 * @RETURN    none
 */
void duer_http_set_max_body_size(duer_http_client_t *p_client, size_t max_body_size);
