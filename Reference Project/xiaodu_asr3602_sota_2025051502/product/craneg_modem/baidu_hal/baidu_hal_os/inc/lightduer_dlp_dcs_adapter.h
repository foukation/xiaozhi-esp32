...
extern const char *DLP_CHILD_WATCH_CONTROL;
...
/**
 * DESC:set recieved dlp directive cache timeout to remove the duplicate directive.
 * @PARAM[in] timeout, unit:s.
 * @RETURN:   0 when success, negative when fail.
 */
duer_s32_t duer_dlp_direct_cache_timeout_set(duer_u32_t timeout_s);

/**
 * DESC:set recieved dlp directive cache length.
 * @PARAM[in] length.
 * @RETURN:   0 when success, negative when fail.
 */
duer_s32_t duer_dlp_direct_cache_max_set(duer_u32_t len);
 ...
 */
baidu_json *duer_event_to_dcs_dlp(baidu_json *root);
