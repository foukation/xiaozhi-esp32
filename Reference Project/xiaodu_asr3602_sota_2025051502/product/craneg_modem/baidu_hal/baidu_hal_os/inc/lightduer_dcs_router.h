...
typedef void (*dcs_system_hook_handler)(baidu_json *directive);
typedef void (*dcs_directive_process_finish_handler)();
...
/**
 * Register callback when all directives of an event have finished processing.
 *
 * @PARAM[in] cb: the callback to be registered.
 * @RETURN: none.
 */
void duer_reg_directive_process_finish_cb(dcs_directive_process_finish_handler cb);

/**
 * Unregister the directive_process_finish callback.
 */
void duer_unreg_directive_process_finish_cb();
...
/**
 * DESC:
 * Used to get the status of local player
 * @PARAM[in] none.
 * @RETURN:   none.
 */
duer_bool duer_get_local_player_is_playing();

/**
 * DESC:
 * Used to get the request ID currently used in a transmission
 * @PARAM[in] none.
 * @RETURN:   char *.
 */
char *duer_get_current_request_id(void);
